...
export function createCrannStateHook<TConfig extends Record<string, ConfigItem<any>>>(config: TConfig) {
  return function useCrannState(context?: string) {
    ...
    const [useCrann, get, set, subscribe] = useMemo(() => {
      console.log("useLensorState: Connecting to Crann with context : ", context);
    ...
    const useStateItem = useCallback(<K extends keyof DerivedState<TConfig>>(key: K) => {
      ...
      const [value, setValue] = useState<DerivedState<TConfig>[K]>(get()[key]);
      ...
      const updateValue = useCallback((newValue: DerivedState<TConfig>[K]) => {
    ...
    const setState = useCallback((newState: Partial<DerivedState<TConfig>>) => {
      set(newState);
    }, [set]);
