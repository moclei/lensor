...
const Lense: React.FC<LenseProps> = ({ imageBitmap, onStop }) => {
  ...
  // Add a state for the effective zoom level
  const [effectiveZoom, setEffectiveZoom] = useState(zoom);

  // Calculate the effective zoom based on pixel scaling setting
  useEffect(() => {
    if (pixelScalingEnabled) {
      // Apply device pixel ratio to show true pixel size
      setEffectiveZoom(zoom * window.devicePixelRatio);
    } else {
  ...
  useEffect(() => {
    console.log('[Lense] zoom level: ', zoom);
  }, [zoom]);
  ...
  const updateSelectedPixelColor = useCallback(
    (newColor: string) => {
  ...
  const updateCanvas = useCallback(() => {
    ...
    // Calculate the size of the area to capture from the source image
    const captureSize = CANVAS_SIZE / effectiveZoom;

    // Get the position of the lens on screen
    const canvasRect = mainCanvasRef.current.getBoundingClientRect();

    // Calculate the center point of the lens
    const lensCenter = {
    ...
    console.log('Lens center:', lensCenter);
    console.log('Mouse position:', mousePos);
    console.log('Capture size:', captureSize);
    console.log('Scale:', scale);

    // Try a manual vertical offset to see if it helps
    // Adjust this value to experiment with different offsets
    const manualYOffset = 100; // Try different values: 20, 50, 100, etc.
  ...
  useEffect(() => {
    if (active && imageBitmap) {
      console.log(
        'useEffect(active && imageBitmap), Initialize lense graphics'
      );
      console.log(
        'active && imageBitmap, getCanvasCenter() called: ',
        getCanvasCenter()
      );
  ...
  const drawGrid = useCallback(() => {
    ...
    const spacing = scale * effectiveZoom;
  ...
  }, [mousePos, effectiveZoom, fisheyeOn, initialDrawComplete]);
